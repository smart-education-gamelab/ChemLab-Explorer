...
//  -----------------------------------------------------------------------------------------------
    ...
    //  Most of this is passed in
        /*
                ...
                VertexSH = SampleSH(input.normalWS.xyz);
    ...
    //  So this is all that has to be done
        ...
        // Old code
            ...
            input.bakedGI = SAMPLE_GI(IN.uvMainAndLM.zw, IN.uvMainAndLM.zw, VertexSH, input.normalWS);
        #else
            input.bakedGI = SAMPLE_GI(IN.uvMainAndLM.zw, VertexSH, input.normalWS);
        #endif

        // New code - test
        // Terrain engine does not undef LIGHTMAP_ON if the renderer is set to "Probes" instead of light maps

        // #if (!defined(LIGHTMAP_ON)) && (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
        // input.bakedGI = 1 * SAMPLE_GI(
        //     0,                                                  // VertexSH
        //     GetAbsolutePositionWS(input.positionWS),            // PostionAWS
        //     input.normalWS,              
        //     viewDirWS,
        //     input.positionCS                                    // PositionSS
        // );

        // #if defined(DYNAMICLIGHTMAP_ON)
        //     input.bakedGI = SAMPLE_GI(IN.uvMainAndLM.zw, IN.uvMainAndLM.zw, VertexSH, input.normalWS);
        // #elif (defined(_FORCEPROBEVOLUMES)) && (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
        // //  In this case the macro is not defined properly, so we call the function directly
        //     input.bakedGI = SampleProbeVolumePixel(
        //         VertexSH,
        ...
        //  Go parallel
            ...
            mweights = saturate((mweights - mthreshold ) / mtransition);
            ...
            splatControl = (sumSplat == 0.0) ? 0.0 : mweights / sumHeight;
